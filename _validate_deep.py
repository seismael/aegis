...
def test_init_creates_rules():
    ...
    assert os.path.exists(os.path.join(root, ".aegis", "rules")), ".aegis/rules/ missing"
    assert os.path.exists(os.path.join(root, ".aegis", "config.yaml")), "config.yaml missing"
...
def test_status_json_fields():
    ...
    assert data["active_violations"] > 0, f"expected violations, got {data['active_violations']}"
...
def test_check_strict():
    rc, out, err = run(["uv", "run", "aegis", "check", "--strict", "--exit-code"], cwd=root)
    assert rc == 1, f"strict rc={rc} (expected 1)"

def test_check_phase_filter():
    rc, out, err = run(["uv", "run", "aegis", "check", "--phase", "pre-commit", "--exit-code"], cwd=root)
    assert rc in (0, 1), f"phase filter rc={rc}"

def test_check_category_filter():
    rc, out, err = run(["uv", "run", "aegis", "check", "--category", "security", "--exit-code"], cwd=root)
    rule_ids = set(re.findall(r'\(([\w-]+)\)', out))
    assert "sec-no-eval" in rule_ids, "security filter didn't return sec rules"
    non_sec = [r for r in rule_ids if not r.startswith("sec-") and not r.startswith("bp-")]
    # Security category should only return security rules
    assert len(non_sec) == 0 or all(
        r.startswith(("semantic-",)) for r in non_sec
...
def test_check_staged():
    ...
    rc, out, err = run(["uv", "run", "aegis", "check", "--staged", "--phase", "on-demand"], cwd=root)
    log(out[:500])
    rule_ids = set(re.findall(r'\(([\w-]+)\)', out))
    assert "sec-no-subprocess-shell" in rule_ids, f"staged didn't detect shell=True. Rules: {rule_ids}"
...
def test_check_clean_after_baseline():
    ...
    log(f"after baseline: {len(rule_ids)} violations ({len(sec_rules)} sec, {len(non_sec_rules)} non-sec)")
    # Non-security violations should be baselined (exempt from check output)
    assert len(non_sec_rules) == 0, f"non-security violations not baselined: {non_sec_rules}"
...
def test_check_exit_code_after_baseline():
    ...
    sec_rules = [r for r in rule_ids if r.startswith("sec-")]
    non_sec_rules = [r for r in rule_ids if not r.startswith("sec-")]
    log(f"--exit-code after baseline: {len(rule_ids)} violations ({len(sec_rules)} sec, {len(non_sec_rules)} non-sec)")
    assert len(non_sec_rules) == 0, f"non-security violations not baselined: {non_sec_rules}"
...
def test_evolve_suppress():
    rc, out, err = run([
        "uv", "run", "aegis", "evolve", "sec-no-eval",
        "--action", "suppress", "--rationale", "test"
    ], cwd=root)
...
def test_rule_install():
    ...
    if rc != 0:
        assert "already installed" in out.lower(), f"unexpected failure: rc={rc}, out={out[:200]}"
...
def test_graph_analyzer_src_layout():
    ...
    rule = Rule(
        id="arch-layer-violation", category=RuleCategory.ARCHITECTURE,
        engine_type=EngineType.GRAPH, query="disallowed_import",
        metadata={"source": "domain", "target": "infrastructure"},
        description="test", severity=Severity.HIGH, mode="block",
    )
    ga = GraphAnalyzer()
    ...
    assert "infrastructure" in violations[0].file or "infrastructure" in str(violations[0].description)
...
def test_regex_analyzer_multiline():
    ...
    rule = Rule(
        id="bp-explicit-exceptions", category=RuleCategory.BEST_PRACTICES,
        engine_type=EngineType.REGEX, query="(?m)^\\s*except\\s*:",
        description="test", severity=Severity.HIGH, mode="warn", language="py",
    )
    analyzer = RegexAnalyzer()
    violations = analyzer.analyze_file(
        "test.py", "def f():\n    try:\n        pass\n    except:\n        pass\n", [rule]
    )
    assert len(violations) == 1, f"expected 1 multiline violation, got {len(violations)}"
...
def test_regex_analyzer_no_multiline():
    ...
    rule = Rule(
        id="bp-explicit-exceptions", category=RuleCategory.BEST_PRACTICES,
        engine_type=EngineType.REGEX, query="^\\s*except\\s*:",
        description="test", severity=Severity.HIGH, mode="warn", language="py",
    )
    analyzer = RegexAnalyzer()
    violations = analyzer.analyze_file(
    ...
    assert len(violations) == 0, f"expected 0 without (?m), got {len(violations)} (bug: line-anchored matched string-start only)"
