...
async def test_get_context_resource(kernel, tmp_path):
    # Setup mock rules
    rule1 = Rule(id="RULE1", description="Critical Rule", severity=Severity.CRITICAL, engine_type=EngineType.REGEX)
    rule2 = Rule(id="RULE2", description="High Rule", severity=Severity.HIGH, engine_type=EngineType.REGEX)
    rule3 = Rule(id="RULE3", description="Medium Rule", severity=Severity.MEDIUM, engine_type=EngineType.REGEX)
    rule4 = Rule(id="RULE4", description="Low Rule", severity=Severity.LOW, engine_type=EngineType.REGEX)
    ...
    kernel.evaluation.evaluate_file.return_value = [
        ArchitecturalViolation(file="test.py", line=1, rule_id="RULE1", description="Violation")
    ]
