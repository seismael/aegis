...
def test_gemini_harness_install():
    ...
    with patch("pathlib.Path.exists", return_value=True):
        with patch("builtins.open", mock_open(read_data=mock_config)) as mocked_file:
            ...
            write_calls = [call for call in mocked_file().write.call_args_list]
...
def test_gemini_harness_deploy_workspace_instructions_path():
    with patch("pathlib.Path.write_text") as mock_write:
        with patch("pathlib.Path.__truediv__", side_effect=lambda self, other: self if other == "GEMINI.md" else Path(str(self)+"/"+other)) as mock_div:
            harness = GeminiHarness(Path("/tmp"))
