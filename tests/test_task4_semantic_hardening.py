...
from aegis.domain.policy.models import Rule, Severity, EngineType, RuleCategory
...
async def test_validate_architecture_compliance_with_semantic_rules():
    ...
    kernel.semantic.build_rubric.return_value = "### 🧩 Semantic Grading Rubric for `test.py`\nIntent: No PII logged"
...
async def test_validate_architecture_compliance_with_both_violations():
    ...
    violation = ArchitecturalViolation(file="test.py", line=1, rule_id="reg-1", description="Found print")
...
def test_semantic_analyzer_heuristic():
    ...
    # Violation
    violations = analyzer.analyze_semantic("test.py", "log_info('user email is test@example.com')", [rule])
