...
class TestPluginScaffold:
    """Tests for PluginScaffold.create."""

    def test_creates_plugin_file(self, tmp_path):
        path = PluginScaffold.create(str(tmp_path), "my-analyzer")
    ...
    def test_invalid_name_raises(self, tmp_path):
        with pytest.raises(ValueError, match="Invalid plugin name"):
            PluginScaffold.create(str(tmp_path), "")

    def test_name_with_spaces_raises(self, tmp_path):
        with pytest.raises(ValueError, match="Invalid plugin name"):
            PluginScaffold.create(str(tmp_path), "my plugin")

    def test_duplicate_name_raises(self, tmp_path):
        PluginScaffold.create(str(tmp_path), "test")
    ...
    def test_creates_plugin_dir(self, tmp_path):
        ...
        PluginScaffold.create(str(sub), "custom")
