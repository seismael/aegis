...
class TestAdvancedPluginArchitecture:
    ...
    def workspace(self, tmp_path):
        ...
        plugin_src = os.path.join(os.getcwd(), ".aegis", "plugins", "dead_module_detector.py")
        with open(plugin_src, "r", encoding="utf-8") as f:
            plugin_content = f.read()
        (plugins_dir / "dead_module_detector.py").write_text(plugin_content, encoding="utf-8")
        
        # 3. Create a project structure
        src_dir = tmp_path / "src"
        src_dir.mkdir()
        (src_dir / "main.py").write_text("import src.useful\ndef start(): pass", encoding="utf-8")
    ...
    def test_custom_remediation_provider(self, workspace):
        ...
        assert "architectural surface area" in prompt
