...
class TestInternalPrivacyPlugin:
    ...
    def workspace(self, tmp_path):
        ...
        plugin_src = os.path.join(os.getcwd(), ".aegis", "plugins", "internal_privacy.py")
        with open(plugin_src, "r", encoding="utf-8") as f:
            plugin_content = f.read()
        (plugins_dir / "internal_privacy.py").write_text(plugin_content, encoding="utf-8")
        
        # Private module
        core_dir = tmp_path / "src" / "myapp" / "core"
        core_dir.mkdir(parents=True)
        (core_dir / "secret_logic.py").write_text("class Secret: pass", encoding="utf-8")
        
        # Authorized consumer
        (core_dir / "container.py").write_text("from myapp.core import secret_logic", encoding="utf-8")
        
        # Unauthorized consumer
        api_dir = tmp_path / "src" / "myapp" / "api"
        api_dir.mkdir(parents=True)
        (api_dir / "public_endpoint.py").write_text("from myapp.core import secret_logic", encoding="utf-8")
