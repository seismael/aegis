...
Use-case: Identifying and cleaning up dead code in large professional repositories.
"""
...
class DeadModulePlugin(CustomAnalyzerInterface):
    ...
    def register_rules(self) -> list[Rule]:
        ...
        return [
            Rule(
                ...
                description="This module is not imported by any other module in the project.",
    ...
    def analyze_project(
        ...
        for root, _, files in os.walk(root_dir):
            if any(x in root.split(os.sep) for x in ["venv", ".venv", ".git", "__pycache__", ".aegis"]):
                continue
        ...
        # (Excluding common entry points like main.py or __init__.py)
            ...
            if not is_imported:
                violations.append(
                    ArchitecturalViolation(
                        ...
                        description=(
                            f"Dead Module Detected: '{mod}' is not imported by any other module. "
                            f"Consider removing it or verifying its entry point."
                        ),
                        severity="LOW"
                    )
                )
    ...
    def provide_remediation(self, violation: ArchitecturalViolation, rule: Rule) -> str | None:
        if violation.rule_id == "dead-module-detection":
            return (
                "Dead Module Detected: This module appears to be dead code. "
                "Verify if it is an external entry point (e.g., a script or lambda handler). "
                "If not, it should be deleted to reduce architectural surface area."
            )
        return None
