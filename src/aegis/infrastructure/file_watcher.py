...
class FileWatcher:
    """Polls a directory for Python file changes and fires callbacks.
    ...
    def _iter_python_files(self) -> dict[str, float]:
    ...
    def watch(
        ...
        """
        known = self._iter_python_files()
        try:
            while True:
                time.sleep(interval)
                current = self._iter_python_files()
